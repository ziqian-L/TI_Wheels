...
void OLED_Init(void)
{
    ...
    //使能GPIOB、GPIOC、GPIOD、GPIOG时钟
    RCC->AHB1ENR |= (1<<1 | 1<<2 | 1<<3 | 1<<6);
    //初始化PB2、PB15
    //普通输出模式、挽推输出、100MHz、上拉
    GPIOB->MODER    |= (1<<(2*2) | 1<<(2*15));
    GPIOB->OTYPER   |= (0<<2 | 0<<15);
    GPIOB->OSPEEDR  |= (3<<(2*2) | (uint32_t)3<<(2*15));
    GPIOB->PUPDR    |= (1<<(2*2) | 1<<(2*15));
    GPIO_ResetBits(GPIOB,15);//作为电源使用
	//初始化PD5、PD14
    GPIOD->MODER    |= (1<<(2*5) | 1<<(2*14));
    GPIOD->OTYPER   |= (0<<5 | 0<<14);
    GPIOD->OSPEEDR  |= (3<<(2*5) | 3<<(2*14));
    GPIOD->PUPDR    |= (1<<(2*5) | 1<<(2*14));
    //初始化PC13
    GPIOC->MODER    |= 1<<(2*13);
    GPIOC->OTYPER   |= 0<<13;
    ...
    //初始化PD8、PD10
    //普通输出模式、挽推输出、100MHz、上拉
    GPIOD->MODER    |= (1<<(2*8) | 1<<(2*10));
    GPIOD->OTYPER   |= (0<<8 | 0<<10);
    GPIOD->OSPEEDR  |= (3<<(2*8) | 3<<(2*10));
    GPIOD->PUPDR    |= (1<<(2*8) | 1<<(2*10));
    GPIO_SetBits(GPIOD,10);//作为电源使用
    //初始化PE8、PE10、PE12、PE14
    //普通输出模式、挽推输出、100MHz、上拉
    GPIOE->MODER    |= (1<<(2*8) | 1<<(2*10) | 1<<(2*12) | 1<<(2*14)); 
    GPIOE->OTYPER   |= (0<<8 | 0<<10 | 0<<12 | 0<<14); 
    GPIOE->OSPEEDR  |= (3<<(2*8) | 3<<(2*10) | 3<<(2*12) | 3<<(2*14)); 
    GPIOE->PUPDR    |= (1<<(2*8) | 1<<(2*10) | 1<<(2*12) | 1<<(2*14));     
